        ...
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item class="flex-item" style="flex: 2;">
            <el-select v-model="formInline.searchColumn" placeholder="请选择查询列">
              <el-option label="账号" value="driver_id"></el-option>
  ...
  methods: {
    ...
    addData() {
      ...
      this.$refs.addform.validate((valid, errors) => {
        if (valid) {
          axios.post('http://106.75.217.80:3334/bus-sys/adddriver_auth', this.addform)
            .then((response) => {
    ...
    editData() {
      ...
      this.$refs.editform.validate((valid, errors) => {
        if (valid) {
          axios.post('http://106.75.217.80:3334/bus-sys/editdriver_auth', this.editform)
            .then((response) => {
              let data = response.data;
              if (data.success) {
                this.fetchDrivers(); // 重新获取司机数据列表
    ...
    handleDelete(row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        axios.post('http://106.75.217.80:3334/bus-sys/deletedriver_auth', row)
          .then((response) => {
    ...
    fetchDrivers() {
      this.loading = true;
      axios.get('http://106.75.217.80:3334/bus-sys/getdriver_auth') // 假设后端接口路径为 /api/drivers
        .then(response => {
